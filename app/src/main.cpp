...
{
    // Init window
    GLFWwindow* window = initWindow(1920, 1080, "Software renderer tester");
    if (window == nullptr)
        return -1;
    ...
    //Framebuffer framebuffer(320, 180);
    ...
    while (glfwWindowShouldClose(window) == false)
    {
        ...
        if (mouseCaptured)
        {
            ...
            inputs.strafeLeft = ImGui::IsKeyDown(GLFW_KEY_LEFT) || ImGui::IsKeyDown(GLFW_KEY_A);
            inputs.strafeRight = ImGui::IsKeyDown(GLFW_KEY_RIGHT) || ImGui::IsKeyDown(GLFW_KEY_D);
            inputs.moveUp = ImGui::IsKeyDown(GLFW_KEY_SPACE);
            inputs.moveDown = ImGui::IsKeyDown(GLFW_KEY_LEFT_CONTROL) || ImGui::IsKeyDown(GLFW_KEY_RIGHT_CONTROL);
        ...
        // Pass BG Info to Renderer
        rdrSetBackground(renderer, (float*)&framebuffer.clearColor);
